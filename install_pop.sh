...
CHECK_RATE_LIMIT=false  # По умолчанию не проверяем статус ограничения частоты запросов
...
backup_configuration() {
    ...
    if [ "$EXCLUDE_CACHE" = false ]; then
        log "Создан��е полной резервной копии (включая кэш)..."
...
# Функция для проверки статуса ограничения частоты запросов
check_rate_limit_status() {
    log "Проверка статуса ограничения частоты запросов..."
    echo "Проверка статуса ограничения частоты запросов..."
    
    # Проверяем наличие файла node_info.json
    if [ -f "node_info.json" ]; then
        local node_id
        node_id=$(grep -o '"node_id":"[^"]*"' node_info.json | cut -d'"' -f4)
        
        if [ -n "$node_id" ]; then
            echo "ID ноды: $node_id"
            
            # Проверяем, когда была последняя попытка регистрации
            local last_attempt
            if [ -f "pop_output.log" ]; then
                last_attempt=$(grep -o "Rate limit.*once per hour" pop_output.log | tail -1)
                
                if [ -n "$last_attempt" ]; then
                    echo "Последняя попытка регистрации: обнаружено ограничение частоты запросов"
                    
                    # Получаем время последней попытки
                    local last_attempt_time
                    last_attempt_time=$(stat -c %Y pop_output.log)
                    local current_time
                    current_time=$(date +%s)
                    local time_diff
                    ...
                    if [ $minutes_left -gt 0 ]; then
                        echo "Осталось примерно $minutes_left минут до снятия ограничения"
                    else
                        echo "Ограничение частоты запросов должно быть снято. Вы можете попробовать запустить ноду снова."
                    fi
                else
                    echo "Информация о последней попытке регистрации не найдена"
                fi
            else
                echo "Лог-файл pop_output.log не найден"
            fi
        else
            echo "ID ноды не найден в node_info.json"
        fi
    else
...
main() {
    ...
    if [[ "$show_logs" =~ ^[Yy]$ ]]; then
        ...
        # Проверяем, запущена ли нода
        if pgrep -f "pop --ram" > /dev/null || systemctl is-active --quiet pop; then
            log "Установка и настройка Pop успешно завершены. Нода запущена."
            echo "Установка и настройка Pop успешно завершены. Нода запущена."
        else
            log "Установка Pop завершена, но нода не запущена."
            echo "Установка Pop завершена, но нода не запущена."
            echo "Возможно, это связано с ограничением частоты запросов (rate limit)."
            echo "Рекомендуется подождать 1 час и запустить ноду снова командой:"
            echo "cd $(pwd) && sudo ./pop --ram $RAM --max-disk $DISK --cache-dir download_cache --pubKey $PUB_KEY --enable-80-443"
        fi
        ...
        echo "Для проверки статуса ограничения частоты запросов используйте команду:"
        echo "bash $(basename "$0") --check-rate-limit"
    fi
}
...
if [ "$SHOW_LOGS_ONLY" = true ]; then
    ...
    show_node_logs
elif [ "$CHECK_RATE_LIMIT" = true ]; then
    # Если скрипт запущен с параметром --check-rate-limit, проверяем статус ограничения частоты запросов
    check_rate_limit_status
else
